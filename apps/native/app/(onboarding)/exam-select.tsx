...
const EXAMS = [
  { id: "enem" as const, title: "ENEM 2024", subtitle: "Exame Nacional", icon: <EnemIcon /> },
  { id: "fuvest" as const, title: "FUVEST (USP)", subtitle: "Estadual São Paulo", icon: <FuvestIcon /> },
  { id: "unicamp" as const, title: "UNICAMP", subtitle: "Estadual Campinas", icon: <UnicampIcon /> },
];

export default function ExamSelectScreen() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const storedExam = useOnboardingStore((s) => s.selectedExam);
  const { setExam } = useOnboardingActions();
  const [selected, setSelected] = useState<Exam>(storedExam ?? "enem");
    ...
    <View style={styles.screen}>
      ...
      <View style={[styles.bottomSection, { paddingBottom: insets.bottom + 24 }]}>
        <View style={styles.hintRow}>
          ...
          <Text style={styles.hintText}>Escolha seu foco principal para continuarmos.</Text>
        </View>
        <View style={styles.buttonWrapper}>
          <PrimaryButton
            ...
            onPress={() => {
              setExam(selected);
              router.push("/(onboarding)/prep-questions");
            }}
          />
        </View>
      </View>
