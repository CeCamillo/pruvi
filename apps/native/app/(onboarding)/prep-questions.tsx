...
export default function PrepQuestionsScreen() {
  ...
  const stored = useOnboardingStore((s) => s.prepTimeline);
  const { setPrepTimeline } = useOnboardingActions();
  const [selected, setSelected] = useState<PrepTimeline>(stored ?? "3m");
    ...
    <View style={styles.screen}>
      ...
      <View style={[styles.bottomSection, { paddingBottom: insets.bottom + 24 }]}>
        <PrimaryButton
          label="PRÓXIMO"
          onPress={() => {
            setPrepTimeline(selected);
            router.push("/(onboarding)/difficulty-questions");
          }}
        />
      </View>
    </View>
