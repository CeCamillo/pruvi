...
describe("calculateSm2", () => {
  ...
  it("third correct answer → interval = round(6 * easeFactor), repetitions=3", () => {
    const first = calculateSm2({ ...BASE, quality: 4 });
    const second = calculateSm2({ ...first, quality: 4 });
    const third = calculateSm2({ ...second, quality: 4 });
    expect(third.interval).toBe(Math.round(6 * second.easeFactor));
    expect(third.repetitions).toBe(3);
  });

  it("wrong answer (q=0) → resets: repetitions=0, interval=1", () => {
    const progressed = calculateSm2({ ...BASE, quality: 5 });
    const result = calculateSm2({ ...progressed, quality: 0 });
    expect(result.repetitions).toBe(0);
    expect(result.interval).toBe(1);
  });

  it("wrong answer (q=2) → resets but keeps prior easeFactor unchanged", () => {
    const prior = calculateSm2({ ...BASE, quality: 5 });
    const result = calculateSm2({ ...prior, quality: 2 });
    expect(result.repetitions).toBe(0);
    expect(result.interval).toBe(1);
